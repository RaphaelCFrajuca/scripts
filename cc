...
echo "First run!"
mkdir /etc/ccgen/
wget https://raw.githubusercontent.com/BadGuy552/scripts/master/gen
mv gen /etc/ccgen/
git clone https://github.com/benhodgson/baluhn.git luhn 1>/dev/null 2>/dev/null
cd luhn && python setup.py install 1>/dev/null 2>/dev/null
rm -rf luhn
$0 $@
fi
if [ "$1" = "" ]
...
# Esse Script Utiliza Outro Script Feito por MIM Usando o baluhn (python3) para calcular o digito verificador.
separator=$3
